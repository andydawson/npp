ring_model_full <- function() {
    ...
    # build cov matrices
    for (i in 1:2) {
      cov2[i,i] <- sigma2_x_obs
    }
    cov2[1,2] <- tau2 * sigma2_x_obs
    cov2[2,1] <- cov2[1,2]

    for (i in 1:3) {	
      cov3[i,i] <- sigma2_x_obs
    }
    
    cov3[1,2] <- tau3 * sigma2_x_obs
    cov3[1,3] <- tau3 * sigma2_x_obs
    cov3[2,3] <- tau3 * sigma2_x_obs
    cov3[2,1] <- cov3[1,2]
    cov3[3,1] <- cov3[1,3]
    cov3[3,2] <- cov3[2,3]

    # for (i in 1:n1cores) {
    #   logXobs[i1core2m[i]] ~ dnorm(log(X[m2tree[i1core2m[i]], m2ti[i1core2m[i]]]), sqrt(sigma2_x_obs))
    # }
    ...
    # for (i in 1:n3cores) {
    #   n = i3core2m[i]
    #   logXobs[n:(n+2)] ~ dmnorm(log(X[m2tree[n], m2ti[n]]), cov3)
    # }
...
#         for(j in 1:last_time[i]) {
#             X[i, j] ~ dlnorm(beta[i] + beta_t[j] + beta_k[tree_site_id[i]], sd = sig_x)
# 	    # X[i, j] ~ dlnorm(beta[i] + beta_t[j], sd = sig_x)
#         }
#         for(j in (last_time[i]+1):nT) {
#             # BUGS (and NIMBLE by default) should ignore if
